...
public class sparkNLPSentimentAnalysisAdvanced {
...
public static void main(String[] args) {
		...
		SparkConf sparkConf = new SparkConf()
				.setAppName("Java Spark streaming For Kafka - Advanced Analysis Sentiment Pipeline")
				.setMaster("spark://pclocalhost:7077");
      
		
		sparkConf.setJars(new String[] {
		      "hdfs://localhost:9000/jars/spark_kafka/spark.nlp.app-sparkNLPSentimentAnalysisAdvanced_00.jar",
		...
		//sparkConf.setCassandraConf("Test Cluster", "testsparkcassandra", ReadConf.SplitSizeInMBParam.option(32));
		...
		*/
		...
		SparkSession spark = SparkSession.builder()
	            //.config(sparkConf)
				.config(sc.getConf())
	            .config("spark.sql.streaming.checkpointLocation","hdfs://localhost:9000/sparkStreamingStructKafka/sparkappcheckpoints_sparkNLPSentimentAnalysisAdvanced")
	            .config("spark.streaming.stopGracefullyOnShutdown","true")
	            .config("spark.debug.maxToStringFields", 2000)
	            .getOrCreate(); 
	        
		//spark.setCassandraConf("Test Cluster", CassandraConnectorConf.ConnectionHostParam.option("127.0.0.1"));
	 ...
	 */
		...
		/*StreamingQuery query = sqlDF.writeStream()
		  ...
		  .start();
	    */
	   //** start define FileStream source....
		StructType Schema = new StructType()
	    	   ...
	    	   .option("sep",",")
	    	   .option("header",true)
	    	   .csv("hdfs://localhost:9000/devsparkstreaming/datasets/nlp");
	    	   
	    	   
	    	   csvDF.createOrReplaceTempView("csvDFTemp");
	    	   //val sqlCsvDF = spark.sql("SELECT CONCAT(Date,H) as eventId, IF(sum(Trans) > 0,1,0) as label, sum(Users) as Users,sum(PagesViews) as PagesViews, sum(Bounces) as Bounces, sum(Clicks) as Clicks from csvDFTemp GROUP BY eventId")
	    	   Dataset<Row> sqlCsvDF = spark.sql("SELECT * from csvDFTemp");
		//**file stream source defined.
		...
		//PipelineModel mypipemodelloaded = PipelineModel.load("hdfs://localhost:9000/sparkNlpPreTrainedModels/recognize_entities_dl_en_2.4.3_2.4_1584626752821");
		//Dataset<Row> pipmodtransfo_output = mypipemodelloaded.transform(sqlDF);
		
		//set up pipeline stages
		PipelineModel analyze_sentiment_en00 = PipelineModel.load("hdfs://localhost:9000/sparkNlpPreTrainedModels/analyze_sentiment_en_2.4.0_2.4_1580483464667");
		PipelineModel recognize_entities_dl_en01 = PipelineModel.load("hdfs://localhost:9000/sparkNlpPreTrainedModels/recognize_entities_dl_en_2.4.3_2.4_1584626752821");
		//PipelineModel explain_document_dl_en01 = PipelineModel.load("hdfs://localhost:9000/sparkNlpPreTrainedModels/explain_document_dl_en_2.4.3_2.4_1584626657780");
		
		String[] array3 = {"document", "sentence", "token", "checked", "sentiment", "document", "sentence", "token", "embeddings", "ner", "entities"};
		PipelineStage finisher02 = new Finisher()
				.setInputCols(array3)
				.setIncludeMetadata(true);
		 
		Pipeline pipelineNLP = new Pipeline()
			    .setStages(new PipelineStage[] {
			    		analyze_sentiment_en00,
			    		recognize_entities_dl_en01,
			    		finisher02
			    });
	
		
		PipelineModel modelNLP00 = pipelineNLP.fit(sqlCsvDF);
		Dataset<Row> pipmodtransfo_output = modelNLP00.transform(sqlCsvDF);
		
		//pipmodtransfo_output.createOrReplaceTempView("dftemp7777");
		
		//Dataset<Row> sqlDFF = spark.sql("SELECT * FROM dftemp7777");
		
		
		
		/*
		 StreamingQuery	query 7777 = df.writeStream
		 ...
		 */
		/*
		StreamingQuery queryok00 = pipmodtransfo_output.select(
		"id","text",
		//"finished_entities",
		"finished_sentiment"
		)
...
*/
		...
		StreamingQuery queryok01 = pipmodtransfo_output.select(
				...
				.foreachBatch(new VoidFunction2<Dataset<Row> , Long>(){
					@Override
					public void call(Dataset<Row> batchDF, Long v2) throws Exception {
						batchDF.persist();
						batchDF.show();
						//batchDF.createOrReplaceTempView("dftemp999");
						
						//Dataset<Row> sqlDFcass = spark.sql("SELECT id, text, CAST(finished_sentiment AS STRING) AS usen FROM dftemp999");
						//sqlDF777.saveToCassandra("testkeyspace","sparkNLPKafka_0");
						batchDF.write()
							.format("org.apache.spark.sql.cassandra")
							.option("keyspace", "testkeyspace")
							.option("table", "sparknlpkafkatest1")
							.mode("append")
							.save();
		...
		/*
				  .outputMode("update")
				  .option("checkpointLocation", "hdfs://localhost:9000/sparkStreamingStructKafka/sparkappcheckpoints_10")
				  .format("org.apache.spark.sql.cassandra")
				  .option("keyspace", "testkeyspace")
				  .option("table", "sparkNLPKafka")
				  .start();
		*/
		/*StreamingQuery queryok01 = pipmodtransfo_output.select(
				"id","text",
